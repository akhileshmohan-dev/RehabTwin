...
def run_sqlite_migrations(engine: Engine) -> Dict[str, bool]:
    ...
    with engine.begin() as conn:
        ...
        if "frames" in tables:
            ...
            if frames_cols and "image_width" not in frames_cols:
                conn.execute(text("ALTER TABLE frames ADD COLUMN image_width INTEGER"))
                applied["frames"] = True
            if frames_cols and "image_height" not in frames_cols:
                conn.execute(text("ALTER TABLE frames ADD COLUMN image_height INTEGER"))
                applied["frames"] = True
